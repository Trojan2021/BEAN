...
func TestTextWrapping(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    []string{"- First item", "\t1. Sub item 1", "\t2. Sub item 2 is really quite long gee I wonder if this will wrap correctly to the next line it sure would be nice if it did", "\t2. Sub item 3 is really quite long gee I wonder if this will wrap correctly to the next line it sure would be nice if it did", "- Second item is also really long to prove that unindented list items can also wrap in the same way as indented ones"},
